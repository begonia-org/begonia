...
func New(config2 *tiga.Configuration, log *logrus.Logger, endpoint string) GatewayWorker {
	configConfig := config.NewConfig(config2)
	mySQLDao := data.NewMySQL(config2)
	...
	dataOperatorUsecase := biz.NewDataOperatorUsecase(dataOperatorRepo, configConfig, log, gatewayWatcher)
