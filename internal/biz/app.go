...
func GenerateAppid(snowflake *tiga.Snowflake) string {
	appid := snowflake.GenerateIDString()
	return appid
}
func GenerateAppAccessKey() (string, error) {
	...
	defer func() {
		if err != nil {
			...
			if strings.Contains(err.Error(), "Duplicate entry") {
				err = gosdk.NewError(err, int32(api.APPSvrCode_APP_DUPLICATE_ERR), codes.AlreadyExists, "commit_app")
			} else {
				err = gosdk.NewError(err, int32(api.APPSvrCode_APP_CREATE_ERR), codes.Internal, "cache_apps")

			}
		}
	}()
	if apps.Appid == "" {
		apps.Appid = GenerateAppid(a.snowflake)
		apps.AccessKey, _ = GenerateAppAccessKey()
		apps.Secret, _ = GenerateAppSecret()
	}
	apps.Owner = owner
...
func (a *AppUsecase) Get(ctx context.Context, key string) (*api.Apps, error) {
	...
	if err != nil {
		return nil, gosdk.NewError(err, int32(api.APPSvrCode_APP_NOT_FOUND_ERR), codes.NotFound, "get_app")
...
func (a *AppUsecase) Del(ctx context.Context, key string) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), gorm.ErrRecordNotFound.Error()) {
			return gosdk.NewError(err, int32(api.APPSvrCode_APP_NOT_FOUND_ERR), codes.NotFound, "delete_app")
		}
		return gosdk.NewError(err, int32(common.Code_INTERNAL_ERROR), codes.Internal, "delete_app")
...
func (a *AppUsecase) Patch(ctx context.Context, in *api.AppsRequest, owner string) (*api.Apps, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "Duplicate entry") {
			return nil, gosdk.NewError(err, int32(api.APPSvrCode_APP_DUPLICATE_ERR), codes.Internal, "update_app")
		}
		return nil, gosdk.NewError(err, int32(common.Code_INTERNAL_ERROR), codes.Internal, "update_app")
	}
	return app, nil
}
...
func (a *AppUsecase) List(ctx context.Context, in *api.AppsListRequest) ([]*api.Apps, error) {
	...
	if err != nil {
		return nil, gosdk.NewError(err, int32(api.APPSvrCode_APP_NOT_FOUND_ERR), codes.NotFound, "list_app")
	}
	return apps, nil
}
