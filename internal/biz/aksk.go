...
func (a *AccessKeyAuth) AppValidator(ctx context.Context, req *gosdk.GatewayRequest) (string, error) {
	...
	if xDate == "" {
		return "", gosdk.NewError(pkg.ErrAppXDateMissing, int32(api.APPSvrCode_APP_XDATE_MISSING_ERR), codes.Unauthenticated, "app_timestamp")
	}
	if auth == "" {
		return "", gosdk.NewError(pkg.ErrAppSignatureMissing, int32(api.APPSvrCode_APP_AUTH_MISSING_ERR), codes.Unauthenticated, "app_signature")
	}
	if accessKey == "" {
		return "", gosdk.NewError(pkg.ErrAppAccessKeyMissing, int32(api.APPSvrCode_APP_ACCESS_KEY_MISSING_ERR), codes.Unauthenticated, "app_access_key")
	}
	...
	if time.Since(t).Abs() > time.Minute*1 {
		...
		return "", gosdk.NewError(pkg.ErrRequestExpired, int32(api.APPSvrCode_APP_REQUEST_EXPIRED_ERR), codes.DeadlineExceeded, "app_timestamp")
	}
