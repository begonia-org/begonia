...
func NewGatewayMux(user *service.UsersService, config *config.Config) *runtime.ServeMux {
	muxNewOnce.Do(func() {
		GlobalMux = runtime.NewServeMux(runtime.WithMarshalerOption("application/json", &middleware.ResponseJSONMarshaler{JSONPb: runtime.JSONPb{
		...
		// err := initGrpcSvr(ctx, GlobalMux, user, config)
		// if err != nil {
		// 	panic(err)
		// }
	})
