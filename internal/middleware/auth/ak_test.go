...
func TestRequestBeforeErr(t *testing.T) {
	c.Convey("TestRequestBeforeErr", t, func() {
		...
		patch1 = patch1.ApplyFuncReturn((*biz.AccessKeyAuth).GetAppid, "", fmt.Errorf("appid is empty"))
		defer patch1.Reset()
		_, err1 := ak.RequestBefore(context.TODO(), &grpc.UnaryServerInfo{}, nil)
		patch1.Reset()
		c.So(err1, c.ShouldNotBeNil)
		c.So(err1.Error(), c.ShouldContainSubstring, "appid is empty")
		...
		patch2 = patch2.ApplyFuncReturn((*biz.AccessKeyAuth).GetAppid, "dadad", nil)
		defer patch2.Reset()
