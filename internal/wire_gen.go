...
func New(config2 *tiga.Configuration, log logger.Logger, endpoint2 string) GatewayWorker {
	...
	businessRepo := data.NewBusinessRepoImpl(dataData, curd, configConfig)
	businessUsecase := biz.NewBusinessUsecase(businessRepo)
	businessServiceServer := service.NewBusinessService(businessUsecase, log, configConfig)
	tenantRepo := data.NewTenantRepoImpl(dataData, configConfig, curd)
	tenantUsecase := biz.NewTenantUsecase(tenantRepo, businessUsecase, configConfig)
	tenantsServiceServer := service.NewTenantService(tenantUsecase, configConfig, log)
	v2 := service.NewServices(fileServiceServer, authServiceServer, endpointServiceServer, appsServiceServer, systemServiceServer, userServiceServer, businessServiceServer, tenantsServiceServer)
	...
	gatewayServer := server.NewGateway(gatewayConfig, configConfig, v2, pluginsApply)
...
func NewWorker(config2 *tiga.Configuration, log logger.Logger, gw string) GatewayWorker {
	configConfig := config.NewConfig(config2)
	mySQLDao := data.NewMySQL(config2)
	redisDao := data.NewRDB(config2)
	etcdDao := data.NewEtcd(config2)
	...
	dataOperatorUsecase := biz.NewDataOperatorUsecase(dataOperatorRepo, configConfig, log, endpointWatcher, endpointRepo)
	daemonDaemon := daemon.NewDaemonImpl(configConfig, dataOperatorUsecase)
	gatewayConfig := server.NewGatewayConfig(gw)
	fileRepo := data.NewFileRepoImpl(dataData, curd)
	v := file.NewFileUsecase(configConfig, fileRepo)
	fileServiceServer := service.NewFileService(v, configConfig)
	usersAuth := crypto.NewUsersAuth(configConfig)
